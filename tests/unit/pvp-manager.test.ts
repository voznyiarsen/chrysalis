...
import { getAttackSpeed, getCooldown, getDamageMultiplier } from '../../src/pvp-manager';
...
describe('getAttackSpeed', () => {
  test('returns OTHER speed for null/undefined', () => {
    ...
    expect(getAttackSpeed(undefined)).toBe(Constants.WEAPON_ATTACK_SPEEDS.OTHER);
  });
  ...
  test('returns OTHER speed for non-weapon items', () => {
    expect(getAttackSpeed('minecraft:apple')).toBe(Constants.WEAPON_ATTACK_SPEEDS.OTHER);
    expect(getAttackSpeed('minecraft:stick')).toBe(Constants.WEAPON_ATTACK_SPEEDS.OTHER);
  });
